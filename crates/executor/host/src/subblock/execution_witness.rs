...
use rsp_rpc_db::{db::RpcDbTrait, execution_witness::ExecutionWitnessRpcDb};
use std::{
...
impl<P: Provider<Ethereum> + Clone + Debug + 'static> HostExecutor<P> {
    pub(crate) async fn execute_subblock_by_execution_witness_rpc(
        &self,
        block_number: u64,
        ...
        let rpc_db_cache_path = execution_witness_rpc_db_cache_file_path(dump_dir, block_number);

        // setup execution witness Rpc Db for block executor
        let mut rpc_db = ExecutionWitnessRpcDb::new(
            block_number - 1,
        ...
        let parent_state = rpc_db.full_state.state.clone();
        ...
        info!(
            "[bench] execute_subblock_by_execution_witness_rpc total time: {:.3?}",
            total_start.elapsed(),
        );
